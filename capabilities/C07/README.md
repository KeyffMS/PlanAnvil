...
Run the configured unsafe Git command corpus through PlanAnvil guard decisions and verify postconditions remain mandatory.
...
Use `fixture/`, `config/README.md`, `prompt.txt`, and `run-command.txt`. Replace the current BLOCKED `actual.sanitized.json`/`evaluation.json` with the live result, update `capabilities/index.json`, then run `python tools/rehash_capability.py C07` and `python tools/validate_capabilities.py`.
