# C16 — Git capability probe matrix
...
Execute the reversible Git capability probe across supported Codex permission modes and record refs/branches/worktrees/index/commit/hook/cleanup outcomes.
...
Use `fixture/`, `config/README.md`, `prompt.txt`, and `run-command.txt`. Replace the current BLOCKED `actual.sanitized.json`/`evaluation.json` with the live result, update `capabilities/index.json`, then run `python tools/rehash_capability.py C16` and `python tools/validate_capabilities.py`.
