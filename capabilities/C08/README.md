...
Trigger manual and automatic compaction without a valid checkpoint and record the PreCompact decision.
...
Use `fixture/`, `config/README.md`, `prompt.txt`, and `run-command.txt`. Replace the current BLOCKED `actual.sanitized.json`/`evaluation.json` with the live result, update `capabilities/index.json`, then run `python tools/rehash_capability.py C08` and `python tools/validate_capabilities.py`.
